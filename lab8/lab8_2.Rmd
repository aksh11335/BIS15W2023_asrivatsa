...
```{r}
expenditures %>% 
  filter(expenditure=="Medical and Health") %>% arrange(expenditure)
...
```{r}
gene_exp %>% 
  pivot_wider(names_from = "type", #the observations under key will become new columns
...
```{r}
beach_bugs<-read_csv(here("lab8", "data","beachbugs_long.csv"))
beach_bugs
```

2. Use `pivot_wider` to transform the data into wide format.
```{r}
beach_bugs<-beach_bugs %>% 
pivot_wider(names_from = site, #new column names come from the education_level column
                values_from = buglevels) #values come from two separate columns
...
```{r}
beach_bugs<-beach_bugs %>% pivot_longer(-year, names_to = "beaches", values_to = "buglevels")
beach_bugs
```

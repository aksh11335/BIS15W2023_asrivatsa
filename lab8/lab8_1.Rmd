...
```{r}
getwd()
#R sets up working directory as where you open file
```

```{r}
here()
#sets it to be one above where it is
```
...
(1) each variable has its own column?  
Yes each variable has its own column ()
(2) each observation has its own row?  
es
(3) each value has its own cell?  
...
heartrate %>% 
  pivot_longer(-patient, 
               ...
               values_to = "heartrate"#values moved to diff column
...
Each of the day values are not ordered into variables per column, as the values per column are still in days

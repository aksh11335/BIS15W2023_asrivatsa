...
```r
beaches<-read_csv(here("lab8","data", "sydneybeaches.csv"))
```
...
The Data is tidy, as each variable has a colomn, each observation has a row, and each value has its own space. This is in the wide format.
...
```r
meansOfBeaches<-sydneybeaches_long%>%
  group_by(site,year)%>%
  summarise(MeansOfEnterocci_cfu_100ml=mean(enterococci_cfu_100ml,na.rm=T))
```
...
## # A tibble: 66 × 3
## # Groups:   site [11]
##    site         year  MeansOfEnterocci_cfu_100ml
##    <chr>        <chr>                      <dbl>
...
```r
meansOfBeaches%>%
  pivot_wider(names_from = "site", values_from = "MeansOfEnterocci_cfu_100ml")
```
...
sydneybeaches_long%>%
  filter(year=="2018")%>%
  slice_max(enterococci_cfu_100ml)
```
...
##   site             day   month year  enterococci_cfu_100ml
##   <chr>            <chr> <chr> <chr>                 <dbl>
## 1 Little Bay Beach 09    01    2018                   2100
```

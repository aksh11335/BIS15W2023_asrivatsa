...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    ...
    ggplot(aes(x=reorder(Ethnicity, FilteredCountFR), y=FilteredCountFR)) + 
      geom_density_2d(color="black", fill="steelblue", alpha=0.75) +
...
ui <- dashboardPage(
  ...
  dashboardBody(
    fluidRow(
      ...
      box(title = "Plot Options", width = 3,
          selectInput("pop", "Select population",choices = c(unique(wolves$pop)), 
                      selected = wolves$pop[1]),
              hr(),
              helpText("wolf Populations")
      ), # close the first box
      box(
        title = "Relative Abundance %", width = 6,
          plotOutput("plot", width = "600px", height = "500px")
      ) # close the second box
    ) # close the row
  ) # close the dashboard body

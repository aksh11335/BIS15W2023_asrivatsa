...
```
No, there are duck that are not from the wet lands
...
Yes, if there is no population size, that means we cannot determine whether the status needs study or not. In this case, if there is no observed value, then we can't tell if its threatened.
...
Amongst the families available, is there a Strong relationship between eggs per clutch and winter range area?
...
![](midterm1Pt2_files/figure-html/unnamed-chunk-17-1.png)<!-- -->

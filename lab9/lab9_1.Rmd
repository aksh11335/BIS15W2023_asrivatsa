...
Know which plot you are going to use prior to using a plot
## Basics
...
Important that you have categorical and quantitative data
...
```{r}
ggplot(data = iris,mapping = aes(Species, y=Petal.Length))
```
...
```{r}
ggplot(data = iris,mapping = aes(Species, y=Petal.Length))+geom_boxplot()
...
#basically if u wanna use ggplot
```
...
```{r}
homerange %>% 
  ggplot(mapping = aes(log10.mass,log10.hra))+geom_point()
#this is very messy
```
...
```{r}
homerange %>% 
  ggplot(mapping = aes(log10.mass,log10.hra))+geom_jitter()
```

To add a regression (best of fit) line, we just add another layer.
```{r}
homerange %>% 
  ggplot(mapping = aes(log10.mass,log10.hra))+geom_jitter()+geom_smooth(method=lm, se=T)
```
...
```{r}
homerange %>% 
  ggplot(mapping=aes(log10.hra, log10.preymass))+geom_point()+geom_smooth(method=lm, se=T)
#ggplot deletes values
```
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+ #since you specified a specific varaible you use a y variable instad of the count
  geom_col()
```

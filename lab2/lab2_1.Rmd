...
# this gives us a value of 270, totally different than (venom_GT+chiron+veyron)/3.0
#so what we want to do is to concatonate the values
cars<-c(venom_GT, chiron, veyron)
# now we can take the mean of the variables with this concatonation
mean(cars)
```
...
```{r}
my_missing <- c(NA,1,3,4,1,2,12,12,1,2,12,3,5,6,67)
```
...
There is an unidentified value inside the function, resulting in an error.This means that are unknown values in the dataset.

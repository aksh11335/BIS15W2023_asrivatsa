...
AN OBJECT
Way of organizing something=> showsup in the left pannle
## `filter()`
...
#look %in%the length variable to be between 167 and 175
```
...
```{r}
filter(fish,near(scalelength,2, tol=0.25) )
```
...
```{r}
selected_mammals<-select(mammals, family, Genus, species, mass, gestation)
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
filter(selected_mammals, mass>450000)
```

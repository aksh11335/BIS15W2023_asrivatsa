...
homerange %>% 
  ggplot(aes(class,log10.mass))+geom_boxplot()
```
...
homerange %>% 
  ggplot(aes(class,log10.mass,fill=class))+geom_boxplot()
```
...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=5)+ #every point on the plot gets a shape#5(circle sqr)
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "deepskyblue4", bins=40)+ #alpha is the darkness of the color in the histogram, color is the border color, and fill is the type of color there is, bins is howmany bins in the diagram
...
homerange %>% 
  ...
  geom_histogram(aes(y = ..density..), fill = "red", alpha = 0.3, color = "black")+
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(fill="deepskyblue4", alpha  =0.7, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
homerange <- homerange %>% 
             mutate(mass_category = 
                      case_when(log10.mass <= 1.75 ~ "small",
...
```{r}
  ...
  homerange2<-homerange %>% 
             mutate(range_category = 
                      case_when(log10.hra <= 3.65 ~ "small",
                     log10.mass > 3.65 & log10.mass <= 4.59 ~ "medium",
                       log10.mass > 6.02 ~ "large")) 
...
```{r}
homerange2 %>% 
  select(range_category, log10.mass,class) %>%
  filter(!is.na(log10.mass)  &!is.na(range_category)) %>% 
  ggplot(aes(range_category, log10.mass,fill=class))+geom_col()
```
...
homerange2 %>% 
  ...
  ggplot(aes(range_category,log10.mass,fill=class))+
  geom_boxplot()
```

...
## Practice
1. Build a faceted plot that shows a distribution of life expectancy(no log 10 mass** correction) by habitat type.
```{r}
names(ecosphere)
ecosphere %>% 
  ggplot(aes(x=log10_mass))+
...
ecosphere %>% 
  ggplot(aes(x=log10_mass))+
  geom_density()+ 
  facet_grid(life_expectancy~habitat, scales = "free_x") 
+theme(axis.text.x = element_text(angle = 60, hjust = 1))
```

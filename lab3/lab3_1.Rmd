...
#classes are posted, in dataframes. This <dbl> is a numeric
```
...
```{r}
depthData<-c(4.15, 4.13, 4.12, 3.21, 3.23, 3.20, 5.67, 5.65, 5.66)
hsprings<-cbind(hsprings,depthData )
hsprings
```

4. Calculate the mean temperature of all of the temperature measurements.
```{r}
#colMeans(hsprings$temp_C)
#colMeans(hsprings)
mean(hsprings[,1])
```
...
```{r}
write.csv(hsprings, "hhsprings_data.csv", row.names = FALSE)

...
```{r}
spider_list[[3]]#this is a list of objects
#R treats this like vectors
```
...
isn't it annoying to have no names for [element]
```{r}
names<- list.files("data/spiders", pattern = ".csv")
```
...
```{r}
library(lubridate)
#ymd is the format of the input values To CONVERT into the R allowed values
ymd(dateformat1)
mdy(dateformat2)
dmy(dateformat3)
mdy_hms(dateformat4)
